...
def test_date_scraper_scrape_no_games_found(patched_request: MagicMock):
    ...
    assert test_ds.scrape() == [
        ScrapeResult(result_type=ScrapeResultType.multiple_games_link, data=[])
    ]


@patch.object(DateScraper, "_extract_game")
@patch.object(DateScraper, "request")
def test_date_scraper_scrape_multiple_games_found(
    patched_request: MagicMock, patched_extract_game: MagicMock
):
    mock_soup = Mock(name="soup")
    ...
    expected = ScrapeResult(
        result_type=ScrapeResultType.multiple_games_link, data=game_links
    )
