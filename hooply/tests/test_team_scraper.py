...
def test_team_scraper_generate():
    assert TeamRosterScraper.generate_resource(TEST_TEAM, TEST_SEASON) == f'teams/{TEST_TEAM}/{TEST_SEASON}.html'
...
def test_team_scraper_scrape_no_players_found(patched_request: MagicMock):
    ...
    assert test_trs.scrape() == [ScrapeResult(result_type=ScrapeResultType.player, data=[])]


@patch.object(TeamRosterScraper, '_extract_player')
@patch.object(TeamRosterScraper, 'request')
def test_team_scraper_scrape_player_found(patched_request: MagicMock, patched_extract_player: MagicMock):
    mock_soup, mock_roster_div = Mock(name="soup"), Mock(name="roster_div")
    patched_request.return_value = mock_soup

    # Setup mocks to avoid bs4 parsing method
    players = [['Jaylen Brown', 'SF', '6-6', '223'], ['Jayson Tatum', 'SF', '6-8', '210']]
    ...
    assert test_trs.scrape() == [ScrapeResult(result_type=ScrapeResultType.player, data=players)]
